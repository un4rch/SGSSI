...
if [[ "$?" == "1"  ]]; then	
	sudo sh -c 'echo "<VirtualHost '$ipaddr':443>" >> /etc/httpd/conf/httpd.conf'
	...
	sudo sh -c 'echo "SSLCertificateChainFile /etc/pki/tls/extracted/pem/server.csr" >> /etc/httpd/conf/httpd.conf'
